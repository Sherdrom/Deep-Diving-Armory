...
- 当前导入的多张图片。相对路径导出只保存项目相对路径；内嵌图片导出会写入 DataURL。
- 当前部件列表。
...
如果 JSON 里图片使用相对路径，路径按项目根目录计算。工具默认从 `.Tools/GunsmithAnchorEditor/index.html` 向上两级解析这些路径。普通文件选择不会暴露真实本地路径，工具用内置 GunSmith 图片清单按文件名和文件大小识别项目图片。
