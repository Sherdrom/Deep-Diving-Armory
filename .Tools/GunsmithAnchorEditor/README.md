...
- `精灵图`：导入一张或多张 PNG/JPG 等图片。普通导入无法拿到浏览器隐藏的真实本地路径，保存时会写入 DataURL。
- `导入项目图片目录`：选择 GunSmith 项目根目录时，工具会把图片记录为项目相对路径，便于 JSON 进 git 后继续预览。
- `查看图片`：切换上方 source 编辑画布显示的图片，不会修改部件 texture。
- `保存工程`：如果工程是通过支持文件写入的浏览器打开的，会直接写回当前 JSON；否则退化为另存为。
- `工程另存为`：选择新 JSON 位置保存当前工程。
- `打开工程 JSON`：继续之前的工具工程。
- `模式`：切换当前鼠标操作。
- `缩放`：调整上方精灵图画布缩放。
- `背景`：切换深色、白色、绿色背景。
...
- 当前导入的多张图片。项目目录导入的图片保存为项目相对路径；普通导入的图片保存为 DataURL。
- 当前部件列表。
...
如果 JSON 里图片使用相对路径，路径按项目根目录计算。工具默认从 `.Tools/GunsmithAnchorEditor/index.html` 向上两级解析这些路径。
