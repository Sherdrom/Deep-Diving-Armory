...
Hook.Add("FAST_Thermal_On", function(_, _, item)
    if isExecuted.FAST_Thermal_On then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    ...
    isExecuted.FAST_Thermal_On = true
    isExecuted.FAST_Thermal_Off = false
end)

Hook.Add("FAST_Thermal_Off", function(_, _, item)
    if isExecuted.FAST_Thermal_Off then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    ...
    isExecuted.FAST_Thermal_Off = true
    isExecuted.FAST_Thermal_On = false
end)

Hook.Add("FAST_Night_On", function(_, _, item)
    if isExecuted.FAST_Night_On then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    ...
    isExecuted.FAST_Night_On = true
    isExecuted.FAST_Night_Off = false
end)

Hook.Add("FAST_Night_Off", function(_, _, item)
    if isExecuted.FAST_Night_Off then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    ...
    isExecuted.FAST_Night_Off = true
    isExecuted.FAST_Night_On = false
end)

Hook.Add("FAST_Health_On", function(_, _, item)
    if isExecuted.FAST_Health_On then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    ...
    isExecuted.FAST_Health_On = true
    isExecuted.FAST_Health_Off = false
end)

Hook.Add("FAST_Health_Off", function(_, _, item)
    if isExecuted.FAST_Health_Off then return end
    local itemComponent = item.GetComponentString("Wearable")
    if itemComponent.wearableSprites[1].Sprite == nil then return end
    itemComponent.wearableSprites[1].Sprite.SourceRect=Rectangle(202,136,120,107)
    itemComponent.wearableSprites[1].Sprite.RelativeOrigin=Vector2(0.45,0.7)
    isExecuted.FAST_Health_Off = true
    isExecuted.FAST_Health_On = false
end)