...
local function getHeldItemIds(character)
    ...
    local ok, inv = pcall(function() return character.Inventory end)
    if ok and inv then
        local slots = { "RightHand", "LeftHand" }
        for _, slotName in ipairs(slots) do
            local ok2, item = pcall(function()
                return inv:GetItemInLimbSlot(InvSlotType[slotName])
            end)
    ...
    -- 后备：检查 SelectedItem
    if #result == 0 then
        local ok3, selItem = pcall(function() return character.SelectedItem end)
        if ok3 and selItem and selItem.Prefab then
...
local function getMainItemObject(character, mainItemId)
    ...
    for _, slotName in ipairs(slots) do
        ...
        if ok2 and item and item.Prefab then
            if tostring(item.Prefab.Identifier) == mainItemId then
                return item
            end
        end
    end
...
local function getActiveSubItems(mainItem)
    local result = {}
    if not mainItem or not mainItem.OwnInventory then return result end
    ...
    local ok = pcall(function()
        for item in mainItem.OwnInventory.AllItemsMod do
...
local function applyStats(character, stats)
    ...
    for _, stat in ipairs(stats) do
        local ok, err = pcall(function()
            character:ChangeStat(StatTypes[stat.statType], stat.value)
        end)
        if ok then
            applied[stat.statType] = stat.value
        else
            errLog("ERROR: applyStat failed -", stat.statType, stat.value, tostring(err))
        end
    end
    return applied
end
...
local function removeStats(character, statsRecord)
    ...
    for statType, value in pairs(statsRecord) do
        local ok, err = pcall(function()
            character:ChangeStat(StatTypes[statType], -value)
        end)
        if not ok then
            errLog("ERROR: removeStat failed -", statType, tostring(err))
        end
    end
end
...
local function applyFlags(character, flags)
    ...
    for _, flagName in ipairs(flags) do
        local ok, err = pcall(function()
            character:AddAbilityFlag(AbilityFlags[flagName])
        end)
        if ok then
            applied[flagName] = true
        else
            errLog("ERROR: applyFlag failed -", flagName, tostring(err))
        end
    end
    return applied
end
...
local function removeFlags(character, flagsRecord)
    ...
    for flagName, _ in pairs(flagsRecord) do
        local ok, err = pcall(function()
            character:RemoveAbilityFlag(AbilityFlags[flagName])
        end)
        if not ok then
            errLog("ERROR: removeFlag failed -", flagName, tostring(err))
        end
    end
end
...
Hook.Add("think", "AdjustStatvalue.Think", function()
    ...
    for _, char in pairs(Character.CharacterList) do
        if char.ID then
            ...
            local ok, err = pcall(processCharacter, char)
            if not ok then
                dbgLog("ERROR processing", char.Name or "?", err)
            end
        end
    end
...
errLog("========== AdjustStatvalue Loading ==========")
...
for itemId, cfg in pairs(CONFIG.items) do
    ...
    if allValid then
        errLog(string.format("  [OK] %s %s -> %s", itemType, itemId, effectStr))
    else
        errLog(string.format("  [FAIL] %s %s -> %s", itemType, itemId, effectStr))
    end
end

errLog(string.format("Main: %d, Sub: %d | Stats: %d OK / %d Fail | Flags: %d OK / %d Fail | Interval: %.2fs (%d frames)",
    mainCount, subCount,
    validStatCount, invalidStatCount,
    validFlagCount, invalidFlagCount,
    CONFIG.checkInterval, framesPerCheck))
errLog("===============================================")