...
-- 安全获取角色属性，失败返回默认值
local function SafeGet(character, propertyPath, defaultValue)
    if character == nil then return defaultValue end
    
    local success, result = pcall(function()
        -- 简单的属性路径解析 (如 "CharacterHealth.TeamID")
        local obj = character
        for part in string.gmatch(propertyPath, "[^%.]+") do
            if obj == nil then return defaultValue end
            obj = obj[part]
        end
        return obj
    end)
...
function NoFriendlyDamage.IsSameTeam(char1, char2)
    ...
    local team1 = SafeGet(char1, "TeamID", -1)
    local team2 = SafeGet(char2, "TeamID", -1)
