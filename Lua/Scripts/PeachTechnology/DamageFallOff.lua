...
function DistanceCalculator.DetectFalloffProfile(attacker)
    ...
    for afflictionName, profile in pairs(DistanceCalculator.FalloffProfiles) do
        ...
        local affliction = attacker.CharacterHealth.GetAffliction(afflictionName)
        ...
        if affliction ~= nil then
            ...
            if strength > 0.5 then
                DistanceCalculator.Debug(string.format("🎯 匹配成功: %s", tostring(afflictionName)))
                return afflictionName, profile
            else
                DistanceCalculator.Debug(string.format("Affliction '%s' 的 Strength (%.3f) <= 0.5，跳过", 
                    tostring(afflictionName), strength))
            end
        else
...
function DistanceCalculator.GetAllAfflictionsDebug(attacker)
    ...
    for afflictionName, _ in pairs(DistanceCalculator.FalloffProfiles) do
        ...
        if aff ~= nil then
            info = info .. string.format("%s=%.2f | ", tostring(afflictionName), aff.Strength or 0)
        else
            info = info .. string.format("%s=不存在 | ", tostring(afflictionName))
        end
    end
    
    return info ~= "" and info or "无数据"
end
