...
local function CalculateFlyingTime(distance, initialVelocity, linearDamping, gravityscale)
    local gravity = 9.8 * gravityscale
    local tick = 0
    if linearDamping == 0 then
        -- No damping, use basic kinematic equation
        tick = (2 * distance) / initialVelocity
    else
    ...
    -- Adjust for gravity's effect
    tick = tick + (2 * distance * gravity) / (initialVelocity ^ 2)
    return tick/60*250
end
