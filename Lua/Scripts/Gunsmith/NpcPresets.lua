...
local function currentSavedState(item)
    ...
    local value = Hook.Call("DeepGunsmithGetSavedState", item)
    if type(value) == "string" then return value end
    return ""
end

local function warn(message)
    print("[Gunsmith][NpcPresets] " .. tostring(message))
end
...
local function canEnsureQuickPartItems()
    ...
    return Hook.Call("DeepGunsmithCanEnsureQuickPartItem") == true
end

local function xmlPresetName(item)
    if not Hook or not Hook.Call then return "" end
    local value = Hook.Call("DeepGunsmithGetNpcPreset", item)
    if type(value) == "string" then return value end
    return ""
end
    ...
    for path, partId in pairs(selection) do
        ...
        if slotIndex ~= nil then
            ...
            if identifier and identifier ~= "" then
                local ok = Hook.Call("DeepGunsmithEnsureQuickPartItem", item, slotIndex, identifier)
                if ok ~= true then
