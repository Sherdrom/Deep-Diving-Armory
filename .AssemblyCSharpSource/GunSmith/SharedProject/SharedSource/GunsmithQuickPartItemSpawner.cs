        ...
        internal static bool Ensure(Item weaponItem, int slotIndex, string itemIdentifier, bool createNetworkEvent)
        {
                ...
                slotIndex >= weaponItem.OwnInventory.slots.Length)
            {
                DebugConsole.NewMessage(
                    $"[Gunsmith] NPC preset quick-slot item '{itemIdentifier}' could not be inserted because '{weaponItem.Prefab.Identifier.Value}' slot {slotIndex} is unavailable.",
                    Color.Yellow,
                    false);
                return false;
            }
            ...
            if (prefab == null)
            {
                DebugConsole.ThrowError($"[Gunsmith] NPC preset quick-slot item prefab not found: {itemIdentifier}");
                return false;
            }
                ...
                try
                {
                    ...
                    if (!inserted)
                    {
                        DebugConsole.ThrowError(
                            $"[Gunsmith] Failed to put NPC preset item '{itemIdentifier}' into '{weaponItem.Prefab.Identifier.Value}' slot {slotIndex}.");
                        RemoveSpawnedItem(spawned);
                    }
                }
                finally
