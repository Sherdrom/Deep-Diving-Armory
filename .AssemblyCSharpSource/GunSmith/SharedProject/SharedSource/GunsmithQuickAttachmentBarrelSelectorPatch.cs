    ...
    public static class GunsmithQuickAttachmentBarrelSelectorPatch
    {
        ...
        private static bool TryPatchSelectorType(Type type)
        {
            lock (PatchLock)
            {
                ...
                {
                    DebugConsole.ThrowError($"GunSmith QAT VCE bridge found selector property on non-ranged type. type={type.FullName}");
                    return false;
                }

                if (harmonyInstance == null)
                {
                    DebugConsole.ThrowError($"GunSmith QAT VCE bridge found {type.FullName}, but Harmony was not initialized.");
                    return false;
                }
                ...
                {
                    DebugConsole.ThrowError($"GunSmith QAT VCE bridge found {type.FullName}, but property {SelectorPropertyName} getter/setter is missing.");
                    return false;
                }
                ...
                if (postfix == null)
                {
                    DebugConsole.ThrowError("GunSmith QAT VCE bridge failed to find its selector postfix method.");
                    return false;
                }
        ...
        private static void AfterCurrentProjectileSelectedChanged(object __instance)
        {
            ...
            {
                DebugConsole.ThrowError($"GunSmith QAT VCE bridge selector postfix received a non-ranged instance. instance={__instance?.GetType().FullName ?? "null"}");
                return;
            }
        ...
        private static bool TryReadSelectedProjectile(object instance, out int selectedProjectile)
        {
            ...
            if (getter == null || !getter.DeclaringType!.IsInstanceOfType(instance))
            {
                ...
                if (getter == null)
                {
                    DebugConsole.ThrowError($"GunSmith QAT VCE bridge could not resolve {SelectorPropertyName} getter for {instance.GetType().FullName}.");
                    return false;
                }
            }
            ...
            DebugConsole.ThrowError(
                $"GunSmith QAT VCE bridge expected {SelectorPropertyName} to return int, " +
