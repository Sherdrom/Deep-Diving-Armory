    ...
    public static class GunsmithServerHooks
    {
        public static void RegisterLuaHooks(Barotrauma.LuaCs.Compatibility.ILuaCsHook hook)
        {
            ...
            hook.Add("DeepGunsmithRegisterQuickAttachmentBarrelCanvasPoint", args =>
            {
                ...
                {
                    DebugConsole.ThrowError("GunSmith QAT received a malformed server barrel canvas payload. Expected item, key, canvasX, canvasY, outletOffsetX, outletOffsetY, rotation, canvasWidth, canvasHeight, worldScale, worldRotation, worldOffsetX, worldOffsetY.");
                    return null;
                }
                ...
                if (!TryCanvasPointToItemLocal(item, canvasPoint, new Vector2(canvasWidth, canvasHeight), worldScale, worldRotation, new Vector2(worldOffsetX, worldOffsetY), out Vector2 localPosition))
                {
                    DebugConsole.ThrowError(
                        $"GunSmith QAT could not convert a server barrel canvas point to item local coordinates. " +
